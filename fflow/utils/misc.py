...
def infer_optimal_gp_hyperparameters(kernel, X, Y):
  """
  Args:
    X: [batch_size, num_points, x_dim]
    Y: [batch_size, num_points, y_dim]
  Returns:
    dict consists of (hyperparameter_name, value) pairs
  """
  model = gp.GaussianProcessRegressor(kernel=kernel, n_restarts_optimizer=10, optimizer="fmin_l_bfgs_b")
  hyps = []
  for i in range(X.shape[0]):
    model.fit(X[i], Y[i])
    hyp = model.kernel_.get_params()
    hyp = {k: hyp[k] for k in hyp if 'bounds' not in k}
    hyps.append(hyp)
  return hyps
